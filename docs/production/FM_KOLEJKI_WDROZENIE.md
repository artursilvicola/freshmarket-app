...
- **Kontrola po wdrożeniu**: smoke curl OK; kontrola formularza PL/EN w przeglądarce — wyłącznie na koncie testowym „TEST Fresh Market – konto testowe (nie uczestniczy)” (company `09e52206…`, `fm_b2b_enabled=false`, `account_status=suspended`, 0 wyborów); logowanie na to konto wykonuje Artur w panelu przeglądarki (Claude nie wpisuje haseł).
