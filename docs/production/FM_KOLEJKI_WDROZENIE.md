...
- **Niewykonane / do Artura**: logowanie sondy admina hasłem nie powiodło się (`Invalid login credentials` — hasło w `fm-probe.env` do poprawy; kontrola admina wykonana symulacją SQL) → **panel admina w przeglądarce** (kontakty awaryjne FRAC/SPAR/Nasz Sklep, „Dane wejściowe”, zapis szkicu planu → `fm_plan_private`); **panel drugiej aplikacji po 054**; odświeżenie otwartych kart (pasek celowo niezamontowany — stare karty nie zapiszą nowych wyborów do przeładowania, dane bezpieczne).
