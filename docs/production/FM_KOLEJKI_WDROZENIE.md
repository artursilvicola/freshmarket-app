...
## 20. Wdrożenie 055_security_hotfix + front dae3740 — 16.09.2026 20:08–20:25 (zgoda warunkowa Artura, review Codexa dae3740 bez blokerów)

- **Warunki wstępne**: 11 kupców bez flagi udziału włączonych pojedynczo (`profiles.fm26_active=true`, 11 wpisów `audit_log buyer_fm_enabled`, zgoda Artura w czacie); konta testowe tylko do odczytu założone wcześniej przez Artura/Codexa (`test.dostawca@` — firma `suspended`/FM off, `test.kupiec@` — sieć 990901 `active=false`/FM off; audit `test_account_configured`); hasła sond w `FreshMarket-Backups\fm-probe.env`, nie w czacie.
- **Kopia przed**: `FreshMarket-Backups\FM-B2B-20260916-200806-przed-055` (DPAPI, stan 18:10:30 UTC, SHA-256 z bazy = z pliku, Verify OK: 785 wyborów / 156 odpowiedzi / 79 potwierdzeń / 119 ukrytych) + `before.json` (18:12:52 UTC). Wcześniejsze kopie (15:53 Codexa, przed-054, po-054) nietknięte.
- **055**: plik z commitu dae3740 (`sha256 91bb1e49…`, potwierdzony po wklejeniu do SQL Editora przed uruchomieniem), jedna transakcja, dialog „destructive operations” → Run query. Kontrola: 12 funkcji, 8 triggerów, `company_target_retailers` tylko polityki SELECT (`ctr_admin_read`, `ctr_buyer_read`, `ctr_supplier_read`), `fm_prefs` bez polityki 002, `fm_settings` = `fm_settings_public_read` + `fms_admin_write`, `audit_log` = `audit_insert_own_client` + `audit_select_admin`, storage per folder; **`retailer_contacts` = 44, `retailers.buyer_*` = 0 niepustych**, `fm_settings.schedule` = null, kolumna `selection_deadline` (null), `fm_plan_private` 0 wierszy, `audit_log security_hotfix/055`. `fm_backup_inputs('po-055-2026-09-16')` jako sesja admina (SQL, claims): 785/156/105 firm/182 profili/45 sieci.
- **Front**: `main` 467cbe4 → dd8e506 (ff-only z `origin/fix/security-hotfix-2026-09-16`), tag wycofania `prod-rollback-2026-09-16b` = 467cbe4; commit docs miał `[skip ci]` → build wyzwolony `netlify api createSiteBuild`; deploy **`6aaadd04921e472a19774c51`** ready 20:17:09 (commit dd8e506). Smoke bundla `index-CppiRH-i.js`: `fm_set_company_targets`, `retailer_contacts`, `fm_my_schedule`, `fm_plan_private` obecne, `NewVersionBanner` niezamontowany, brak ścieżki legacy DELETE+INSERT; `/version.json` = `dd8e506f007c`.
- **Porównanie po**: `after.json` (18:23:10 UTC) vs `before.json` → `fm-inputs-compare` **exit 0, 10 sekcji bez różnic** (785 wyborów, 156 odpowiedzi, 119 ukrytych, 105 firm FM, 182 profile, 45 sieci, fm_settings). Snapshot `…-201736-po-055` (DPAPI, Verify OK) vs przed-055 narzędziem Codexa: **0 różnic w 16 tabelach decyzyjnych**; `fm_settings` 1 zmiana = `schedule {} → null` + nowa kolumna `selection_deadline: null` (skutek 055, wyjaśniony).
- **Kontrola dostępu** (`fm-permission-probe.mjs`, tylko odczyt, 39 sond): anon — `consent_audit`, `v_admin_*`, `retailer_contacts`, `fm_plan_private`, `fm_inputs_snapshots`, RPC `fm_set_company_targets` → 401/42501; `company_capacity`/`retailers`/`audit_log` → 0 wierszy. **Dostawca testowy**: 45 sieci widocznych, **0 kontaktów** (`buyer_*` puste, `retailer_contacts` null/0), 0 profili kupców, 0 `fm_prefs`, `fm_plan_private` 0, `fm_my_schedule` null, `fm_resps`/wybory tylko własne (0), `legacy_sends` bez adresów, `audit_log` 0. **Kupiec testowy**: analogicznie, profile tylko własny. Sesja **prawdziwego aktywnego dostawcy** (SQL, rollback): 45 sieci, 0 z jakimkolwiek kontaktem, `retailer_contacts` 0, cudze wybory/odpowiedzi 0, własne wybory 11, plan null, audit 0. `v_admin_stats` dla nie-admina = 1 wiersz samych zer (widok agregujący, brak danych). **Admin** (SQL, claims `artur@kjow.pl`): `retailer_contacts` 44 (osadzenie w 44 sieciach), `fm_plan_private` czytelne, `audit_log` czytelne (w tym `security_hotfix/055`), `consent_audit` 142, `company_capacity` 115, snapshot po-055 widoczny.
- **Sonda admina hasłem (20:35, po poprawie hasła w `fm-probe.env`)**: 46/46 PASS — `retailer_contacts` 44 (osadzone w 44 sieciach), `fm_plan_private` czytelne (0 wierszy), `fm_my_schedule` null (brak planu), `consent_audit` 142, `company_capacity` 115, `audit_log` czytelne (0 zapisów wyborów od deployu — nikt jeszcze nie klikał).
- **Niewykonane / do Artura**: **panel admina w przeglądarce** (kontakty awaryjne FRAC/SPAR/Nasz Sklep, „Dane wejściowe”, zapis szkicu planu → `fm_plan_private`); **panel drugiej aplikacji po 054**; odświeżenie otwartych kart (pasek celowo niezamontowany — stare karty nie zapiszą nowych wyborów do przeładowania, dane bezpieczne).
- **Punkt powrotu**: deploy `6aaa69eb3efff20007827d64` (008504d) = tag `prod-rollback-2026-09-16b` (467cbe4) — przywracać deploy, **nie cofać bazy** (fail-closed, §5 notatki). Bez zmiany fazy/terminu, bez przeliczania/publikacji planu, bez maili.
