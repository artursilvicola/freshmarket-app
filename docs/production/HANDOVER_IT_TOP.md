...
| `PAYU_ENV` (`sandbox`/`prod`), `PAYU_POS_ID`, `PAYU_SECOND_KEY`, `PAYU_OAUTH_CLIENT_ID`, `PAYU_OAUTH_CLIENT_SECRET` | **na prod ustawić `prod`** |
...
- ⭐ „star" = sieć główna, **limit 5 × liczba pakietów Business** (`fm_b2b_packages`, 1–5; `src/lib/fm-stars.js`) — po wyczerpaniu puli kolejna automatycznie staje się rezerwową.
- 👍 „thumb" = rezerwowa, **bez limitu**.
- „Gotowy" = ≥ 5 gwiazdek (minimum `FM_STARS_MIN`, niezależne od limitu) → przycisk **Potwierdź wybór** (`fm_selection_confirmed_at`). Licznik i komunikaty pokazują `{{count}}/{{max}}`, po minimum z wolną pulą: „Minimum osiągnięte — możesz dodać jeszcze N ⭐”.
