...
// Only one pair of QR codes per supplier card, alongside the operational instructions.
...
export function meetingResourcesBlock(lang) {
  ...
  return { unbreakable: true, margin: [0, 6, 0, 0], stack: [
    ...
    meetingSponsorsBlock(),
  ] };
}

export function meetingSponsorsBlock() {
  return { columns: SPONSOR_LOGOS.map(s => ({ image: s.dataUri, fit: [66, 20], width: 74, margin: [0, 0, 8, 0] })) };
}
...
export function meetingNoticeBlock(lang, audience) {
  ...
  return { pageBreak: "before", stack: [
    ...
    meetingContactsBlock(lang),
  ] };
}

// Buyer print cards retain organizer contacts without the supplier notice or QR section.
export function meetingContactsBlock(lang) {
  const n = meetingNotice(lang);
  return { unbreakable: true, margin: [0, 5, 0, 0], stack: [heading(n.helpTitle),
