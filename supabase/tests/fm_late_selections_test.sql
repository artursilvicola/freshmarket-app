...
update public.fm_late_resps set zone='remove' where retailer_id=990801;
select pg_temp.ok((select count(*)=1 and min(zone)='remove' from public.fm_late_resps),'remove accepted (migration 20260921052500)');
select pg_temp.denied('update public.fm_late_resps set zone=''maybe''');
