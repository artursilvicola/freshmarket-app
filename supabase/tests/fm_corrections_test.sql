...
select pg_temp.fails($q$select public.fm_commit_correction(10,gen_random_uuid(),'unlock')$q$,'fm_correction_admin_only');
reset role;
...
select pg_temp.fails($q$select public.fm_commit_correction(10,gen_random_uuid(),'unlock')$q$,'permission denied');
reset role;
...
select 'PASS: confirmation writes, swap/move/undo, history, revisions, rejection, approval, roles, unchanged inputs' result;
rollback;
