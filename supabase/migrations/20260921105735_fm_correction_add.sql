-- Add a missing meeting without moving existing cells or changing matching inputs.
-- Apply migration before frontend. No existing draft, history, preference or plan is updated.
-- Lower per-plan allowances may be stored in schedule.meeting_limits, keyed by supplier ID.
-- Supply these through an audited rebuild before using Add on a plan with agreed lower limits.
-- Rollback: keep SQL/history. An older frontend cannot display/undo Add entries; disable edits
-- in the old frontend or roll forward. Do not remove history or restore the old action CHECK.
begin;
alter table public.fm_correction_history drop constraint if exists fm_correction_history_action_check;
alter table public.fm_correction_history add constraint fm_correction_history_action_check
 check(action in ('initialize','swap','move','remove','add','undo','rebuild','load_approved','approve','unlock'));
...
declare d public.fm_correction_drafts%rowtype; h public.fm_correction_history%rowtype;
 ...
 add_company public.companies%rowtype; add_retailer public.retailers%rowtype;
 total_meetings int; allowed_meetings int; queue_count int; queue_cap int; latest_date date;
...
begin
 ...
 if p_action='initialize' then
   ...
   cb:=p_details->'to'->>'cid'; sid:=p_details->'to'->>'sid';
   if jsonb_typeof(p_details->'to'->'pos') is distinct from 'number'
      or coalesce(p_details->'to'->>'pos','') !~ '^[0-9]{1,5}$' then raise exception 'fm_correction_invalid_cell'; end if;
   pb:=(p_details->'to'->>'pos')::int; q:=old->'cq';
   if cb is null or not(q ? cb) or pb>10000 then raise exception 'fm_correction_invalid_cell'; end if;
   if q->cb->>pb is not null then raise exception 'fm_correction_occupied'; end if;
   select * into add_company from public.companies c where c.id::text=sid or c.legacy_fm_id=sid for share;
   if add_company.id is null or add_company.fm_b2b_enabled is distinct from true or coalesce(add_company.account_status,'active')<>'active'
      or sid is distinct from coalesce(nullif(add_company.legacy_fm_id,''),add_company.id::text) then raise exception 'fm_correction_supplier_ineligible'; end if;
   select * into add_retailer from public.retailers r where r.fm26_chain_id=cb for share;
   if add_retailer.id is null or add_retailer.active is false or add_retailer.fm26_active is distinct from true then raise exception 'fm_correction_chain_inactive'; end if;
   if exists(select 1 from jsonb_array_elements_text(q->cb) x(s) where x.s=sid) then raise exception 'fm_correction_duplicate'; end if;
   allowed_meetings:=5*greatest(1,least(5,coalesce(add_company.fm_b2b_packages,1)));
   if old->'meeting_limits' ? sid then
     if jsonb_typeof(old->'meeting_limits'->sid) is distinct from 'number' or (old->'meeting_limits'->>sid) !~ '^[0-9]{1,2}$' then raise exception 'fm_correction_limit_invalid'; end if;
     if (old->'meeting_limits'->>sid)::int not between 1 and 25 then raise exception 'fm_correction_limit_invalid'; end if;
     allowed_meetings:=least(allowed_meetings,(old->'meeting_limits'->>sid)::int);
   end if;
   select count(*) into total_meetings from jsonb_each(q) x cross join lateral jsonb_array_elements_text(x.value) v(s) where v.s=sid;
   if total_meetings>=allowed_meetings then raise exception 'fm_correction_supplier_limit'; end if;
   if exists(select 1 from jsonb_each(q) x cross join lateral jsonb_array_elements_text(x.value) with ordinality v(s,n)
     where v.s=sid and abs(v.n-1-pb)<2) then raise exception 'fm_correction_gap'; end if;
   -- Keep current capacity and buyer responses stable until the atomic draft write.
   lock table public.fm_queue_groups,public.fm_stations,public.fm_resps in share mode;
   if exists(select 1 from public.fm_resps r where r.retailer_id=add_retailer.id and r.supplier_company_id=add_company.id
     and coalesce(nullif(r.zone,''),r.status) in ('remove','rejected')) then raise exception 'fm_correction_buyer_rejected'; end if;
   -- Same date selection as getFmQueueCapacityByRetailer(). No client-supplied capacity.
   select max(event_date) into latest_date from public.fm_queue_groups;
   if exists(select 1 from public.fm_queue_groups where retailer_id=add_retailer.id and event_date=latest_date) then
     select coalesce(sum(g.meetings_per_station),0) into queue_cap from public.fm_queue_groups g
       join public.fm_stations s on s.queue_group_id=g.id and s.active
       where g.retailer_id=add_retailer.id and g.event_date=latest_date and g.active;
   else queue_cap:=60; end if;
   select count(*) into queue_count from jsonb_array_elements_text(q->cb) x(s) where x.s is not null;
   if queue_count>=queue_cap then raise exception 'fm_correction_capacity'; end if;
   while jsonb_array_length(q->cb)<=pb loop q:=jsonb_set(q,array[cb],(q->cb)||'null'::jsonb); end loop;
   q:=jsonb_set(q,array[cb,pb::text],to_jsonb(sid));
   next:=old;
   -- An eligible firm registered after the draft was built can receive its first meeting.
   if not(next->'res' ? sid) then next:=jsonb_set(next,array['res',sid],'{"m":[],"r":{}}'::jsonb,true); end if;
   next:=fm_corrections_private.reindex(next,q);
   next:=jsonb_set(next,array['cs',cb,'cap'],to_jsonb(queue_cap),true);
   details:=jsonb_build_object('to',jsonb_build_object('cid',cb,'pos',pb,'sid',sid,'company',add_company.name,'chain',add_retailer.name),
     'before_count',total_meetings,'after_count',total_meetings+1,'supplier_limit',allowed_meetings);
   ...
   ca:=p_details->'from'->>'cid'; pa:=(p_details->'from'->>'pos')::int;
   q:=old->'cq';
   if ca is null or not(q ? ca) or pa is null or pa<0 or pa>10000 then
     raise exception 'fm_correction_invalid_cell'; end if;
   a:=q->ca->>pa;
   if a is null or a is distinct from (p_details->'from'->>'sid') then raise exception 'fm_correction_conflict'; end if;
   -- Keep the hole: deletion must never shift other meetings or change their numbers.
   q:=jsonb_set(q,array[ca,pa::text],'null'::jsonb,false);
   next:=fm_corrections_private.reindex(old,q);
   select name into company_name from public.companies co where co.id::text=a or co.legacy_fm_id=a limit 1;
   select name into chain_name from public.retailers r where r.fm26_chain_id=ca limit 1;
   details:=jsonb_build_object('from',jsonb_build_object('cid',ca,'pos',pa,'sid',a,'company',coalesce(company_name,a),'chain',coalesce(chain_name,ca)));
 elsif p_action='undo' then
   select * into h from public.fm_correction_history x
     where action in ('swap','move','remove','add','rebuild','load_approved')
