-- Remove one meeting with revision checks, immutable history and undo.
-- Apply before the frontend. Existing drafts, history and preferences are preserved.
-- Rollback: retain this migration and data; republish the prior frontend. Export the
-- latest draft first. The prior frontend cannot undo/display removals fully.
begin;
...
alter table public.fm_correction_history add constraint fm_correction_history_action_check
 check(action in ('initialize','swap','move','remove','undo','rebuild','load_approved','approve','unlock'));
...
begin
 ...
 if p_action='initialize' then
   ...
   select * into h from public.fm_correction_history x
     where action in ('swap','move','remove','rebuild','load_approved')
